...
const CreateProducts = () => {
  ...
  const {content, setContent, createProducts, getCategories, getSubCategories, loading, setLoading} = context;
...
const isFormValid = () => {
  return (
    ...
    productData.eggType?.trim() &&
    productData.preparationTime?.trim() &&
    content?.trim() && // productDesc from Jodit editor
    productData.metaTitle?.trim() &&
    productData.metaDesc?.trim() &&
...
setLoading(false)
  ...
  return (
    <>
        <h1 className='' style={{fontFamily:'revert-layer'}}>Create Product</h1>
       <div class="m-3">
        <h2 className="text-start">Meta Details</h2>
    <input type="text" class="form-control p-2 mb-3" placeholder='Enter Meta Title' name='metaTitle' onChange={handleChange}/>
    <input type="text" class="form-control p-2" placeholder='Enter Meta Description' name='metaDesc' onChange={handleChange}/>
...
>
  ...
  <option value="custom">Custom</option> */}
    ...
    <input type="text" name='productName' class="form-control p-2 mb-3" placeholder='Enter Product Name' onChange={handleChange}/>
...
<div className='d-flex'>
    <input name='preparationTime' type="number" class="form-control p-2" placeholder='Enter Preparation Time' onChange={handleChange}/>
    <span className='justify-content-center align-item-center m-2'>hours</span>
</div>


{/* <button type="button"  class="btn btn-primary my-4 float-end px-3" onClick={handleSubmit}>Create Product</button> */}
<button type="button" disabled={!isFormValid() || loading}  class="btn btn-primary my-4 float-end px-3" onClick={handleSubmit}>
  {loading ? (
  <>
    <span
...
/>
        <button type="button" disabled={loading} class="btn btn-primary" onClick={ async ()=>{ await uploadToCloudinary(croppedImage); setUploadImgModal(false)}} >
          {loading ? (
  <>
    <span
      className="spinner-border spinner-border-sm me-2"
